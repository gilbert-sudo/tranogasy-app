...
export const useMap = () => {
  ...
  function findLocationsWithinDistance(locations, center, distance, placeName) {
    ...
    locations.forEach((location) => {
      ...
      if (dist <= distance) {
        if (placeName) {
          const locationName = `${location.fokontany} ${location.commune}`.toLowerCase();
          const simScore = checkTextSimilarity(locationName, placeName) * 10;
          nearbyLocations.push({ location: location, distance: dist, simScore });
        } else {

// CommuneAutosuggestInput.js
import React, { Component } from "react";

class CommuneAutosuggestInput extends Component {
  constructor() {
    super();
    this.state = {
      value: "",
  ...
  onChange = (event) => {
    ...
    const suggestions = data.filter((item) => {
      const itemText =
        `${item.commune} ${item.district} `.toLowerCase();
          ...
          <button
            ...
            onClick={() =>
              this.handleSuggestionClick(
                ...
                ", " +
                item.district.toUpperCase() + "( " + item.province + " )",
                item // Pass the item's _id as the second argument
              )
            }
          >
            <b>{`${item.commune.charAt(0).toUpperCase() + item.commune.slice(1)
              }, ${item.district.toUpperCase()}`}</b>{" "}
            <br />
            <small>{`${item.region.toUpperCase()}`}</small>{" "}
            {`(${item.province})`}
          </button>
        ))}
      </div>
      ...
      <>
        ...
        {suggestions && suggestions.length === 0 && value !== "" &&
          ...
          >
            ...
            >
              <small><b>La saisie ne correspond à aucun commune.</b></small>
              <br />
...
export default CommuneAutosuggestInput;

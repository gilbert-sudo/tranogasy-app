// DistrictAutosuggestInput.js
import React, { Component } from "react";

class DistrictAutosuggestInput extends Component {
  constructor() {
    super();
    this.state = {
      value: "",
  ...
  onChange = (event) => {
    ...
    const suggestions = data.filter((item) => {
      const itemText =
        `${item.district} ${item.region}`.toLowerCase();
    ...
    return (
      ...
      >
        {suggestions.map((item) => (
          ...
          >
            <b>{`${item.district.charAt(0).toUpperCase() + item.district.slice(1)
              }`}</b>{" "}
            <br />
      ...
      <>
        ...
        {suggestions && suggestions.length === 0 && value !== "" &&
          ...
          >
            ...
            >
              <small><b>La saisie ne correspond à aucun district.</b></small>
              <br />
...
export default DistrictAutosuggestInput;
